...
namespace GreenBox.Missile.Threading
{
    ...
    public class Dispatcher
    {
        ...
        private readonly ManualResetEventSlim _processEvent;
        ...
        private Dispatcher()
        {
            ...
            _processEvent = new ManualResetEventSlim(false);
        }
        ...
        public void Invoke(Action callback, DispatcherPriority priority, CancellationToken cancellationToken, TimeSpan timeout)
        {
            ...
            if (priority == DispatcherPriority.Send && !cancellationToken.IsCancellationRequested && CheckAccess() && DisableCounter == 0)
            {
        ...
        private object FinalInvoke(DispatcherPriority priority, TimeSpan timeout, Delegate method, object args, int argumentMode)
        {
            ...
            if (timeout.TotalMilliseconds < 0 && timeout.TotalMilliseconds != -1)
                throw new ArgumentOutOfRangeException("timeout");


            if (priority == DispatcherPriority.Send && CheckAccess() && DisableCounter == 0)
            {
                SynchronizationContext oldContext = SynchronizationContext.Current;
