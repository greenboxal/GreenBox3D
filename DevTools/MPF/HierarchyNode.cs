        ...
        //, IVsBuildStatusCallback 
    {
        ...
        public virtual VsStateIcon StateIconIndex
        {
            get
            {
                ...
                {
                    IVsSccManager2 sccManager = ProjectMgr.Site.GetService(typeof(SVsSccManager)) as IVsSccManager2;
        ...
        {
            get { return hasDesigner; }
            set { hasDesigner = value; }
        }
        ...
        {
            get { return excludeNodeFromScc; }
            set { excludeNodeFromScc = value; }
        }
        ...
        {
            get { return hasParentNodeNameRelation; }
            set { hasParentNodeNameRelation = value; }
        }

        protected bool SourceDraggedOrCutOrCopied
        {
            get { return sourceDraggedOrCutOrCopied; }
            set { sourceDraggedOrCutOrCopied = value; }
        }
        ...
        public virtual void AddChild(HierarchyNode node)
        {
            ...
            {
                if (ProjectMgr.CompareNodes(node, n) > 0)
                    break;
                previous = n;
            }
        ...
        public virtual object GetProperty(int propId)
        {
            ...
            switch ((__VSHPROPID)propId)
            {
                ...
                case __VSHPROPID.VSHPROPID_ItemDocCookie:
                    if (docCookie != 0)
                        return (IntPtr)docCookie; //cast to IntPtr as some callers expect VT_INT
                    break;
        ...
        protected virtual int AddNewFolder()
        {
            ...
            try
            {
                ...
                ErrorHandler.ThrowOnFailure(projectMgr.GenerateUniqueItemName(hierarchyId, String.Empty, String.Empty,
                                                                              out newFolderName));
                ...
                {
                    IVsUIHierarchyWindow uiWindow = UIHierarchyUtilities.GetUIHierarchyWindow(projectMgr.Site,
                                                                                              SolutionExplorer);
                    ...
                    if (uiWindow != null)
                    {
                        ...
                        ErrorHandler.ThrowOnFailure(uiWindow.ExpandItem(projectMgr.InteropSafeIVsUIHierarchy,
                                                                        child.hierarchyId, EXPANDFLAGS.EXPF_SelectItem));
        ...
        protected virtual int ExcludeFromProject()
        {
            Debug.Assert(ProjectMgr != null,
                         "The project item " + ToString() +
                         " has not been initialised correctly. It has a null ProjectMgr");
            Remove(false);
            return VSConstants.S_OK;
        }
        ...
        protected internal virtual StringBuilder PrepareSelectedNodesForClipBoard()
        {
            ...
            if (solution != null)
            {
                ErrorHandler.ThrowOnFailure(solution.GetProjrefOfItem(ProjectMgr.InteropSafeIVsHierarchy, hierarchyId,
                                                                      out projref));
        ...
        protected virtual int ExecCommandIndependentOfSelection(Guid cmdGroup, uint cmdId, uint cmdExecOpt, IntPtr vaIn,
                                                                IntPtr vaOut, CommandOrigin commandOrigin,
                                                                out bool handled)
        {
            handled = false;
                                                  ...
                                                  CommandOrigin commandOrigin)
        {
            ...
            bool handled = false;
            int returnValue = ExecCommandIndependentOfSelection(cmdGroup, cmdId, cmdExecOpt, vaIn, vaOut, commandOrigin,
                                                                out handled);
            if (handled)
            {
                return returnValue;
        ...
        protected internal virtual void GetSccFiles(IList<string> files, IList<tagVsSccFilesFlags> flags)
        {
            ...
            tagVsSccFilesFlags flagsToAdd = (firstChild != null && (firstChild is DependentFileNode))
                                    ...
                                    out int cancelled)
        {
            ...
            if (docData == IntPtr.Zero)
            {
                string errorMessage = string.Format(CultureInfo.CurrentCulture,
                                                    SR.GetString(SR.CanNotSaveFileNotOpeneInEditor,
                                                                 CultureInfo.CurrentUICulture), node.Url);
            ...
            try
            {
                ...
                else
                {
                    ...
                    bool emptyOrSamePath = String.IsNullOrEmpty(docNew) ||
                    ...
                    if (saveAs)
                    {
                        ...
                        if ((returnCode == (int)OleConstants.OLECMDERR_E_CANCELED || returnCode == VSConstants.E_ABORT))
                        {
                            ...
                            if (ff != null)
                            {
                                returnCode = shell.SaveDocDataToFile(VSSAVEFLAGS.VSSAVE_SilentSave, ff,
            ...
            for (HierarchyNode child = firstChild; child != null; child = child.NextSibling)
            {
                    ...
                    String.Compare(child.VirtualNodeName, name, StringComparison.OrdinalIgnoreCase) == 0)
                {
                    ...
                    // If it is a foldernode then it has a virtual name but we want to find folder nodes by the document moniker or url
                         ...
                         (NativeMethods.IsSamePath(child.GetMkDocument(), name) ||
        ...
        internal bool AddFileToNodeFromProjectReference(string projectRef, HierarchyNode targetNode)
        {
            ...
            int addResult = targetNode.ProjectMgr.AddItem(targetNode.ID, VSADDITEMOPERATION.VSADDITEMOP_OPENFILE, null,
