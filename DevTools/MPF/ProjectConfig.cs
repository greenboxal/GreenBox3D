...
namespace Microsoft.VisualStudio.Project
{
        ...
        IVsCfgBrowseObject
    {
        ...
        public ProjectConfig(ProjectNode project, string configuration)
        {
            ...
            IntPtr projectUnknown = Marshal.GetIUnknownForObject(ProjectMgr);
            try
            {
                IVsProjectFlavorCfgProvider flavorCfgProvider =
            ...
            if (null != persistXML)
            {
                this.project.LoadXmlFragment(persistXML, DisplayName);
            }
        }
        ...
        {
            project.PrepareBuild(configName, clean);
        }
        ...
        {
            if (!project.QueryEditProjectFile(false))
            {
                throw Marshal.GetExceptionForHR(VSConstants.OLE_E_PROMPTSAVECANCELLED);
            }

            string condition = String.Format(CultureInfo.InvariantCulture, ConfigProvider.configString, ConfigName);
        ...
        private void SetPropertyUnderCondition(string propertyName, string propertyValue, string condition)
        {
            ...
            {
                project.BuildProject.SetProperty(propertyName, propertyValue);
                return;
            }
            ...
            foreach (MSBuildConstruction.ProjectPropertyGroupElement group in project.BuildProject.Xml.PropertyGroups)
            {
            ...
            foreach (MSBuildConstruction.ProjectPropertyElement property in newGroup.PropertiesReversed)
                // If there's dupes, pick the last one so we win
            {
        ...
        public virtual int get_ProjectCfgProvider(out IVsProjectCfgProvider p)
        {
            ...
            project.GetCfgProvider(out cfgProvider);
            if (cfgProvider != null)
            {
        ...
        public virtual int DebugLaunch(uint grfLaunch)
        {
            ...
            try
            {
                ...
                {
                    info.bstrExe = project.GetOutputAssembly(ConfigName);
                }
                else
                {
                ...
                VsShellUtilities.LaunchDebugger(project.Site, info);
            }
            catch (Exception e)
            {
        ...
        {
            CCITracing.TraceCall();
            string assembly = project.GetAssemblyName(ConfigName);
        ...
        {
            if (project == null || project.NodeProperties == null)
            {
                throw new InvalidOperationException();
            }
            return project.NodeProperties.GetProjectItem(out hier, out itemid);
        }
                                                                ...
                                                                out string platformName)
        {
            ...
            string[] splittedCanonicalName = canonicalName.Split(new[] { '|' }, StringSplitOptions.RemoveEmptyEntries);
        ...
        private void GetCfgPropertyPages(CAUUID[] pages)
        {
            ...
            ErrorHandler.ThrowOnFailure(
                hierarchy.GetProperty(VSConstants.VSITEMID_ROOT, (int)__VSHPROPID2.VSHPROPID_CfgPropertyPagesCLSIDList,
                                      out variant), new[] { VSConstants.DISP_E_MEMBERNOTFOUND, VSConstants.E_NOTIMPL });
    ...
    public class BuildableProjectConfig : IVsBuildableProjectCfg
    {
        ...
        private void NotifyBuildEnd(MSBuildResult result, string buildTarget)
        {
            ...
            foreach (IVsBuildStatusCallback cb in callbacks)
            {
                ...
                finally
                {
                    ...
                    bool shouldRepaintReferences = (buildTarget == null || buildTarget == MsBuildTarget.Build ||
        ...
        private void RefreshReferences()
        {
            ...
            IReferenceContainer referenceContainer = config.ProjectMgr.GetReferenceContainer();
