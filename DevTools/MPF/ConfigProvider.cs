...
namespace Microsoft.VisualStudio.Project
{
    ...
    public class ConfigProvider : IVsCfgProvider2, IVsProjectCfgProvider, IVsExtensibleObject
    {
        ...
        private List<KeyValuePair<KeyValuePair<string, string>, string>> newCfgProps =
        ...
        {
            return new ProjectConfig(project, configName);
        }
        ...
        public virtual int OpenProjectCfg(string projectCfgCanonicalName, out IVsProjectCfg projectCfg)
        {
            ...
            Debug.Assert(project != null && project.BuildProject != null);
        ...
        public virtual int AddCfgsOfCfgName(string name, string cloneName, int fPrivate)
        {
            ...
            string condition = (cloneName == null
                                    ? String.Empty
                                    : String.Format(CultureInfo.InvariantCulture, configString, cloneName).Trim());

            // Get all configs
            List<ProjectPropertyGroupElement> configGroup =
                new List<ProjectPropertyGroupElement>(project.BuildProject.Xml.PropertyGroups);
            ...
            if (configToClone != null)
            {
                ...
                newConfig = project.ClonePropertyGroup(configToClone);
            ...
            else
            {
                ...
                newConfig = project.BuildProject.Xml.AddPropertyGroup();
            ...
            //add the output path
            string outputBasePath = ProjectMgr.OutputBaseRelativePath;
            ...
            newConfig.AddProperty("OutputPath",
                                  Path.Combine(outputBasePath, name) + Path.DirectorySeparatorChar.ToString());
        ...
        public virtual int DeleteCfgsOfCfgName(string name)
        {
            ...
            foreach (string config in configs)
            {
                if (String.Compare(config, name, StringComparison.OrdinalIgnoreCase) == 0)
                {
                    ...
                    foreach (ProjectPropertyGroupElement element in project.BuildProject.Xml.PropertyGroups)
                    {
        ...
        /// <param name="names">
        ///     On input, an allocated array to hold the number of configuration property names specified by celt. This parameter can also be a null reference if the celt parameter is zero.
        ...
        public virtual int RenameCfgsOfCfgName(string old, string newname)
        {
            ...
            foreach (ProjectPropertyGroupElement config in project.BuildProject.Xml.PropertyGroups)
            {
                // Only care about conditional property groups
        ...
        {
            string platforms = ProjectMgr.BuildProject.GetPropertyValue(ProjectFileConstants.AvailablePlatforms);
        ...
        private string[] GetPropertiesConditionedOn(string constant)
        {
            ...
            project.BuildProject.ConditionedProperties.TryGetValue(constant, out configurations);
