...
namespace Microsoft.VisualStudio.Project
{
                                                 ...
                                                 IBuildDependencyOnProjectContainer
    {
        ...
        [CLSCompliant(false)]
        public override int GetNestedHierarchy(UInt32 itemId, ref Guid iidHierarchyNested, out IntPtr ppHierarchyNested,
                                               out uint pItemId)
        {
                                     ...
                                     out int pfCancelled)
        {
            ...
            {
                return
                    ErrorHandler.ThrowOnFailure(base.SaveItem(dwSave, silentSaveAsName, itemid, punkDocData,
                                                              out pfCancelled));
            }
        }
        ...
        public virtual int OpenChildren()
        {
            ...
            try
            {
                ...
                IVsFireSolutionEvents fireSolutionEvents =
        ...
        protected internal void RunVsTemplateWizard(ProjectElement element, bool silent)
        {
            ...
            OAProject oaProject = GetAutomationObject() as OAProject;
            ...
            try
            {
                ...
                IVsExtensibility extensibilityService = (IVsExtensibility)GetService(typeof(IVsExtensibility));
                wizardResult result = extensibilityService.RunWizardFile(template, 0, ref wizParams);
        ...
        private RegisteredProjectType GetRegisteredProject(ProjectElement element)
        {
            ...
            DTE dte = ProjectMgr.Site.GetService(typeof(DTE)) as DTE;
            Debug.Assert(dte != null, "Could not get the automation object from the services exposed by this project");

            if (dte == null)
                throw new InvalidOperationException();

            RegisteredProjectType registeredProjectType =
                RegisteredProjectType.CreateRegisteredProjectType(projectFactoryGuid);
        ...
        protected virtual void ReloadNestedProjectNode(NestedProjectNode node)
        {
            ...
            try
            {
                ...
                try
                {
                    __VSCREATEPROJFLAGS flags = __VSCREATEPROJFLAGS.CPF_NOTINSLNEXPLR | __VSCREATEPROJFLAGS.CPF_SILENT |
                ...
                catch (Exception e)
                {
                    ...
                    {
                        string message = (String.IsNullOrEmpty(e.Message))
        ...
        private void OnNestedProjectFileChangedOnDisk(object sender, FileChangedOnDiskEventArgs e)
        {
            ...
            if (!Utilities.IsInAutomationFunction(Site))
            {
                ...
                string message = String.Format(CultureInfo.CurrentCulture,
                                               SR.GetString(SR.QueryReloadNestedProject, CultureInfo.CurrentUICulture),
                                               moniker);
                string title = string.Empty;
