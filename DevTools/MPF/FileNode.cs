    ...
    public class FileNode : HierarchyNode
    {
        ...
        public override string Caption
        {
            get
            {
                ...
                string caption = ItemNode.GetMetadata(ProjectFileConstants.LinkedIntoProjectAt);
        ...
        public override string Url
        {
            get
            {
                string path = ItemNode.GetMetadata(ProjectFileConstants.Include);
        ...
        {
            ISingleFileGenerator generator = CreateSingleFileGenerator();
        ...
        public override int SetEditLabel(string label)
        {
            ...
            for (HierarchyNode n = Parent.FirstChild; n != null; n = n.NextSibling)
            {
                if (n != this && String.Compare(n.Caption, label, StringComparison.OrdinalIgnoreCase) == 0)
                {
                    ...
                    throw new InvalidOperationException(String.Format(CultureInfo.CurrentCulture,
                                                                      SR.GetString(SR.FileOrFolderAlreadyExists,
                                                                                   CultureInfo.CurrentUICulture), label));
                }
            }
            ...
            string strRelPath = Path.GetFileName(ItemNode.GetMetadata(ProjectFileConstants.Include));
            if (
                               ...
                               StringComparison.OrdinalIgnoreCase) != 0)
            {
                ...
                string message = SR.GetString(SR.ConfirmExtensionChange, CultureInfo.CurrentUICulture, new[] { label });
        ...
        protected internal override int SetEditLabel(string label, string relativePath)
        {
            ...
            else
            {
                        ...
                        String.Compare(Path.GetFileName(newName), Path.GetFileName(Url), StringComparison.Ordinal) != 0))
                {
                    throw new InvalidOperationException(String.Format(CultureInfo.CurrentCulture,
                                                                      SR.GetString(
                                                                          SR.FileCannotBeRenamedToAnExistingFile,
                                                                          CultureInfo.CurrentUICulture), label));
                }
            ...
            string oldrelPath = ItemNode.GetMetadata(ProjectFileConstants.Include);
            ...
            if (cmdGroup == VsMenus.guidStandardCommandSet97)
            {
                ...
                switch ((VsCommands)cmd)
                {
                    case VsCommands.ViewCode:
                        return ((FileDocumentManager)GetDocumentManager()).Open(false, false, VSConstants.LOGVIEWID_Code,
                                                                                out windowFrame,
                    ...
                    case VsCommands.Open:
                        return ((FileDocumentManager)GetDocumentManager()).Open(false, false, WindowFrameShowAction.Show);
        ...
        protected override int AfterSaveItemAs(IntPtr docData, string newFilePath)
        {
            ...
            string oldCanonicalDirectoryName = new Uri(Path.GetDirectoryName(GetMkDocument())).LocalPath;
            ...
            bool isSameFile = NativeMethods.IsSamePath(newFilePath, Url);

            // Currently we do not support if the new directory is located outside the project cone
            string projectCannonicalDirecoryName = new Uri(ProjectMgr.ProjectFolder).LocalPath;
            ...
            else
            {
                ...
                if (targetContainer != null && (targetContainer is FileNode))
                {
                    ...
                    errorMessage = String.Format(CultureInfo.CurrentCulture,
                                                 SR.GetString(SR.FileAlreadyExistsAndCannotBeRenamed,
                                                              CultureInfo.CurrentUICulture),
                                                 Path.GetFileNameWithoutExtension(newFilePath));
                    throw new InvalidOperationException(errorMessage);
                }
            ...
            if (targetContainer == null)
            {
                ...
                string relativeUri = PackageUtilities.GetPathDistance(ProjectMgr.BaseURI.Uri, newDirectoryUri);
                Debug.Assert(!String.IsNullOrEmpty(relativeUri) && relativeUri != newDirectoryUri.LocalPath,
                             "Could not make pat distance of " + ProjectMgr.BaseURI.Uri.LocalPath + " and " +
                             newDirectoryUri);
            ...
            string oldrelPath = ItemNode.GetMetadata(ProjectFileConstants.Include);
            string oldName = Path.Combine(ProjectMgr.ProjectFolder, oldrelPath);
            SuspendFileChanges sfc = new SuspendFileChanges(ProjectMgr.Site, oldName);
            sfc.Suspend();
        ...
        {
            bool fileExist = IsFileOnDisk(Url);

            if (!fileExist && showMessage && !Utilities.IsInAutomationFunction(ProjectMgr.Site))
            {
                string message = String.Format(CultureInfo.CurrentCulture,
                                               SR.GetString(SR.ItemDoesNotExistInProjectDirectory,
                                                            CultureInfo.CurrentUICulture), Caption);
        ...
        protected virtual FileNode RenameFileNode(string oldFileName, string newFileName, uint newParentId)
        {
            ...
            ErrorHandler.ThrowOnFailure(ProjectMgr.AddItemWithSpecific(newParentId,
                                                                       VSADDITEMOPERATION.VSADDITEMOP_OPENFILE, null, 0,
                                                                       file, IntPtr.Zero, 0, ref emptyGuid, null,
                                                                       ref emptyGuid, result));
            FileNode childAdded = ProjectMgr.FindChild(newFileName) as FileNode;
                                                            ...
                                                            IList<tagVsSccFilesFlags> flags)
        {
            ...
            foreach (HierarchyNode node in GetChildNodes())
            {
        ...
        private void RenameCaseOnlyChange(string newFileName)
        {
            ...
            string include = ItemNode.Item.EvaluatedInclude;
