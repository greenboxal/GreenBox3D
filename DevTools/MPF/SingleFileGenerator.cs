...
namespace Microsoft.VisualStudio.Project
{
    ...
    internal class SingleFileGenerator : ISingleFileGenerator, IVsGeneratorProgress
    {
        ...
        public virtual void RunGenerator(string document)
        {
                ...
                ErrorHandler.Succeeded(hier.ParseCanonicalName(document, out itemid)))
            {
                ...
                if (VerifyFileDirtyInRdt(document, out rdtHier, out perDocData, out cookie))
                {
                    ...
                    FileNode node = (FileNode)projectMgr.NodeFromItemId(itemid);
        ...
        protected internal virtual void InvokeGenerator(FileNode fileNode)
        {
            ...
            SingleFileGeneratorNodeProperties nodeproperties =
                fileNode.NodeProperties as SingleFileGeneratorNodeProperties;
            ...
            try
            {
                if (!runningGenerator)
                {
                    ...
                    int generateTempPE;
                    SingleFileGeneratorFactory factory = new SingleFileGeneratorFactory(projectMgr.ProjectGuid,
                                                                                        projectMgr.Site);
                    ErrorHandler.ThrowOnFailure(factory.CreateGeneratorInstance(customToolProgID,
                                                                                out generateDesignTimeSource,
                                                                                out generateSharedDesignTimeSource,
                                                                                out generateTempPE, out generator));
                    ...
                    {
                        customToolNamespace = ComputeNamespace(moniker);
                    }
                    ...
                    string inputFileContents = GetBufferContents(moniker, out stream);

                    ErrorHandler.ThrowOnFailure(generator.Generate(moniker, inputFileContents, customToolNamespace,
                                                                   output, out outPutSize, this));
        ...
        protected virtual string ComputeNamespace(string projectItemPath)
        {
            ...
            string[] toks = filePath.Split(new[] { ':', '\\' });
        ...
        protected virtual string UpdateGeneratedCodeFile(FileNode fileNode, byte[] data, int size, string fileName)
        {
            ...
            IVsRunningDocumentTable rdt =
                projectMgr.GetService(typeof(SVsRunningDocumentTable)) as IVsRunningDocumentTable;
            ...
            else
            {
                ...
                ProjectItem projectItem = fileNode.GetAutomationObject() as ProjectItem;
            ...
            if (rdt != null)
            {
                ...
                try
                {
                    ...
                    if (ErrorHandler.Failed(hr) || docData == IntPtr.Zero)
                    {
                        ...
                        cookie = 0;
                        docInRdt = false;
                        ILocalRegistry localReg = projectMgr.GetService(typeof(SLocalRegistry)) as ILocalRegistry;
                                          ...
                                          out uint cookie)
        {
            ...
            IVsRunningDocumentTable rdt =
                projectMgr.GetService(typeof(IVsRunningDocumentTable)) as IVsRunningDocumentTable;
            if (rdt != null)
            {
                IntPtr docData;
                uint dwCookie = 0;
        ...
        private bool CanEditFile(string documentMoniker)
        {
            ...
            try
            {
                ...
                IVsQueryEditQuerySave2 queryEditQuerySave =
                    (IVsQueryEditQuerySave2)projectMgr.GetService(typeof(SVsQueryEditQuerySave));
