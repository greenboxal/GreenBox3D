...
namespace Microsoft.VisualStudio.Project
{
    ...
    public partial class ProjectNode : IVsUIHierWinClipboardHelperEvents
    {
        ...
        ///     Current state of the keyboard keys and the mouse modifier buttons. See <seealso cref="IVsHierarchyDropDataTarget" />
        /// </param>
        /// <param name="itemid">Item identifier of the drop data target over which the item is being dragged</param>
        /// <param name="pdwEffect">
        ///     On entry, reference to the value of the pdwEffect parameter of the IVsHierarchy object, identifying all effects that the hierarchy supports.
        ///     On return, the pdwEffect parameter must contain one of the effect flags that indicate the result of the drop operation. For a list of pwdEffects values, see
        ///     <seealso
        ///         cref="DragEnter" />
        /// </param>
        /// <returns>If the method succeeds, it returns S_OK. If it fails, it returns an error code.</returns>
        ...
        ///     Reference to the IDataObject interface on the item being dragged. This data object contains the data being transferred in the drag-and-drop operation.
        ///     If the drop occurs, then this data object (item) is incorporated into the target hierarchy or hierarchy window.
        /// </param>
        /// <param name="grfKeyState">
        ///     Current state of the keyboard and the mouse modifier keys. See <seealso cref="IVsHierarchyDropDataTarget" />
        /// </param>
        /// <param name="itemid">Item identifier of the drop data target over which the item is being dragged</param>
        /// <param name="pdwEffect">
        ///     Visual effects associated with the drag-and drop-operation, such as a cursor, bitmap, and so on.
        ///     The value of dwEffects passed to the source object via the OnDropNotify method is the value of pdwEffects returned by the Drop method
        /// </param>
        /// <returns>If the method succeeds, it returns S_OK. If it fails, it returns an error code. </returns>
        ...
        ///     The value of dwEffects passed to the source object via OnDropNotify method is the value of pdwEffects returned by Drop method.
        /// </param>
        /// <returns>If the method succeeds, it returns S_OK. If it fails, it returns an error code. </returns>
        ...
        internal virtual DataObject PackageSelectionDataObject(bool cutHighlightItems)
        {
            ...
            try
            {
                ...
                IntPtr ptrToItems = PackageSelectionData(sb, false);
                ...
                IntPtr ptrToProjectPath = PackageSelectionData(new StringBuilder(GetMkDocument()), true);
                ...
                {
                    bool first = true;
                    IVsUIHierarchyWindow w = UIHierarchyUtilities.GetUIHierarchyWindow(site, SolutionExplorer);
                    ...
                    if (w != null)
                    {
                        foreach (HierarchyNode node in ItemsDraggedOrCutOrCopied)
                        {
                            ErrorHandler.ThrowOnFailure(w.ExpandItem(InteropSafeIVsUIHierarchy, node.ID,
                                                                     first
                                                                         ? EXPANDFLAGS.EXPF_CutHighlightItem
                                                                         : EXPANDFLAGS.EXPF_AddCutHighlightItem));
                            first = false;
                        }
                    }
                }
                                                       ...
                                                       HierarchyNode targetNode, bool addSiblings)
        {
            ...
            if (itemId != VSConstants.VSITEMID_NIL)
            {
                ...
                string name = Path.GetFileName(source.TrimEnd(new[] { '/', '\\' }));
                ...
                FileProperties fileProperties = variant as FileProperties;
            ...
            try
            {
                ...
                if (dataObject != null)
                {
                    ...
                    IVsUIHierWinClipboardHelper clipboardHelper =
                        (IVsUIHierWinClipboardHelper)GetService(typeof(SVsUIHierWinClipboardHelper));
        ...
        protected internal override bool AllowPasteCommand()
        {
            ...
            try
            {
                ...
                FORMATETC format = DragDropHelper.CreateFormatEtc(DragDropHelper.CF_VSSTGPROJECTITEMS);
        ...
        internal DropDataType ProcessSelectionDataObject(IOleDataObject dataObject, HierarchyNode targetNode)
        {
            ...
            List<string> filesDropped = DragDropHelper.GetDroppedFiles(DragDropHelper.CF_VSSTGPROJECTITEMS, dataObject,
                                                                       out dropDataType);
            ...
            if (dropDataType != DropDataType.None && filesDropped.Count > 0)
            {
                ...
                if (isWindowsFormat)
                {
                    ...
                    int addResult = AddItem(node.ID, VSADDITEMOPERATION.VSADDITEMOP_OPENFILE, null,
