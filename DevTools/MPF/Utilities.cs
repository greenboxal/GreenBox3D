...
namespace Microsoft.VisualStudio.Project
{
    public static class Utilities
    {
        ...
        public static string GetMsBuildPath(IServiceProvider serviceProvider, string version)
        {
            ...
            using (
                RegistryKey root = VSRegistry.RegistryRoot(serviceProvider, __VsLocalRegistryType.RegType_Configuration,
                                                           false))
            {
            ...
            string registryPath = string.Format(CultureInfo.InvariantCulture,
        ...
        public static bool IsInAutomationFunction(IServiceProvider serviceProvider)
        {
            ...
            IVsExtensibility3 extensibility = serviceProvider.GetService(typeof(IVsExtensibility)) as IVsExtensibility3;
        ...
        public static ImageList GetImageList(object imageListAsPointer)
        {
            ...
            if (count > 0)
            {
                ...
                try
                {
                    ...
                    {
                        UnsafeNativeMethods.ImageList_Draw(hImageList, i, handleRefDC, i * 16, 0,
                                                           NativeMethods.ILD_NORMAL);
                    }
                }
                finally
                {
        ...
        public static string SetStringValueFromConvertedEnum<T>(T enumValue, CultureInfo culture)
            where T : struct
        {
            string convertToType = ConvertToType(enumValue, typeof(string), culture) as string;
                                                                 ...
                                                                 string fullProjectPath)
        {
            ...
            List<MSBuild.Project> loadedProject =
                new List<MSBuild.Project>(buildEngine.GetLoadedProjects(fullProjectPath));
            MSBuild.Project buildProject = loadedProject != null && loadedProject.Count > 0 && loadedProject[0] != null
                                               ? loadedProject[0]
                                               : null;
        ...
        internal static MSBuild.Project ReinitializeMsBuildProject(MSBuild.ProjectCollection buildEngine,
                                                                   string fullProjectPath,
                                                                   MSBuild.Project exitingBuildProject)
        {
            ...
            where T : struct
        {
            ...
            if (attributes != null && attributes.Length == 1)
            {
                ...
                PropertyPageTypeConverterAttribute converterAttribute =
                    (PropertyPageTypeConverterAttribute)attributes[0];
        ...
        private static bool IsFilePartInValid(string filePart)
        {
            ...
            if (!String.IsNullOrEmpty(extension))
            {
                ...
                Regex unsafeCharactersRegex = new Regex(regexToUseForExtension,
            ...
            Regex unsafeFileNameCharactersRegex = new Regex(regexToUseForFileName,
                                                            RegexOptions.Compiled | RegexOptions.IgnoreCase |
                                                            RegexOptions.CultureInvariant);
        ...
        internal static bool TryGetActiveConfigurationAndPlatform(IServiceProvider serviceProvider,
                                                                  IVsHierarchy hierarchy, out string configuration,
                                                                  out string platform)
        {
            if (serviceProvider == null)
            {
