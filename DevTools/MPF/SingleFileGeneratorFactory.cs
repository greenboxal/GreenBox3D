...
namespace Microsoft.VisualStudio.Project
{
    ...
    public class SingleFileGeneratorFactory : IVsSingleFileGeneratorFactory
    {
        ...
        private class GeneratorMetaData
        {
            ...
            {
                get { return useDesignTimeCompilationFlag; }
            ...
            get
            {
                if (baseGeneratorRegistryKey == null)
                {
                    using (RegistryKey root = VSRegistry.RegistryRoot(__VsLocalRegistryType.RegType_Configuration))
                    {
                        if (null != root)
                        {
                            string regPath = "Generators\\" + ProjectGuid.ToString("B");
        ...
        public virtual int CreateGeneratorInstance(string progId, out int generatesDesignTimeSource,
                                                   out int generatesSharedDesignTimeSource, out int useTempPEFlag,
                                                   out IVsSingleFileGenerator generate)
        {
            ...
            ErrorHandler.ThrowOnFailure(GetGeneratorInformation(progId, out generatesDesignTimeSource,
                                                                out generatesSharedDesignTimeSource, out useTempPEFlag,
                                                                out genGuid));
                                                   ...
                                                   out Guid guidGenerator)
        {
            ...
            if (!generatorsMap.ContainsKey(progId))
            {
                ...
                RegistryKey tempBaseGeneratorKey = BaseGeneratorsKey;
